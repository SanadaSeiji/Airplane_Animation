...
float quat[4];
float yaw = -90.0f; //cam inside head of the plane
float pitch = 5.0f;
float roll = 0.0f;
...
void display() {
	...
	if (isFirstPerson == TRUE) {
		skyboxM = local1R;
		skyboxM = local1T * skyboxM;
	}

	glUniformMatrix4fv(cube_V_location, 1, GL_FALSE, inverse(skyboxM).m);
	...
	pointCount = generateObjectBufferTeapot("../Meshs/se.obj");
	...
	if (isFirstPerson == FALSE) {
		view = translate(view, vec3(0.0, 2.0, -20.0));
		   ...
		   // place camera slightly upon plane
		view = rotate_y_deg(view, -90.0);
		view = rotate_z_deg(view, 0.0);
		view = translate(view, vec3(0.0, -1.3, -2.5));
	}
	...
	if (isFirstPerson == FALSE) local = local1R * local;
	...
	if (isFirstPerson == FALSE) local = local1T*local;
	...
	//------------------------------------------------
	// child of hierarchy
	pointCount = generateObjectBufferTeapot("../Meshs/CosP.obj");

	// child of hierarchy
	mat4 local2 = identity_mat4();
	R = rotate_x_deg(R, 60.0);
	local2 = R*local2;
	local2 = translate(local2, vec3(-3.15, 0.35, 0));
	// global of the child is got by pre-mul3iplying the local of the child by the global of the parent
	mat4 global2 = global*local2;
...
void init()
{
	...
	shaderTx = CompileShaders(shaderTx, "../Shaders/txVS.txt", "../Shaders/txFS.txt");
	...
	create_cube_map("../Textures/sky/negz.jpg", "../Textures/sky/posz.jpg", "../Textures/sky/posy.jpg",
		"../Textures/sky/negy.jpg", "../Textures/sky/negx.jpg", "../Textures/sky/posx.jpg", &cubeTex);
